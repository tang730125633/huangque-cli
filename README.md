...
先用 `hq run assets --input @assets.json --json` 查本人资产中的 `audio_file`，用 `hq run video-avatars --json` 查本人可用的 `avatar_id`。本地参考视频只接受 MP4、MOV 或 WebM，使用绝对路径并需显式确认。单个普通图片或视频上传没有 CLI 产品容量上限；服务端按当前账号的短期私有上传总数（最多 20 个）和总容量（最多 2 GiB）验收，不会静默压缩文件：
